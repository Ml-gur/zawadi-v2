  ...
  return (
    <div className="min-h-[100dvh] bg-canvas text-cream">
      ...
      <div className="max-w-[960px] mx-auto px-6 py-12">
        ...
        <h1 className="font-display text-4xl font-semibold text-cream tracking-tight mb-2">
        ...
        <div className="relative mb-10">
          <input
            ...
            className="w-full p-4 pl-12 bg-off-black border border-hairline rounded-lg text-sm text-cream placeholder:text-muted focus:border-accent-green focus:ring-1 focus:ring-accent-green/40 outline-none transition-colors"
          />
          <svg className="absolute left-4 top-1/2 -translate-y-1/2 text-muted w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          {search && (
            <button
              onClick={() => setSearch('')}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-muted hover:text-cream cursor-pointer"
            >
              Clear
            </button>
          )}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-lg text-cream mb-2">No results found for "{search}"</p>
            <p className="text-xs text-muted">Try different keywords or browse the categories below.</p>
          </div>
        ) : (
          <div className="space-y-10">
            {filtered.map((cat, catIdx) => (
              <div key={catIdx}>
                <h2 className="text-xs font-semibold text-cream uppercase tracking-wider mb-4 flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {cat.items.map((faq, idx) => {
                    ...
                    return (
                      <div key={faqId} className={`border rounded-lg transition-colors duration-300 overflow-hidden ${isOpen ? 'bg-off-black border-hairline' : 'bg-transparent border-hairline/60 hover:border-hairline'}`}>
                        <button
                        ...
                        >
                          <span className={`font-semibold text-xs uppercase tracking-wide transition-colors duration-200 ${isOpen ? 'text-accent-green' : 'text-cream'}`}>
                            {faq.q}
                          </span>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-300 ${isOpen ? 'border-accent-green/50 text-accent-green rotate-180' : 'border-hairline text-muted group-hover:border-hairline'}`}>
                        ...
                        <div className={`transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[500px] opacity-100 border-t border-hairline/60' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                          <div className="p-5 text-xs text-muted leading-relaxed font-light whitespace-pre-line">
                            {faq.a}
                          </div>
                        </div>
                      </div>
